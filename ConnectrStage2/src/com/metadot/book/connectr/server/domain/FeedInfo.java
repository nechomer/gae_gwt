...
/**
 * The FeedInfo persistence-capable class is used in conjunction with the
 * FeedIndex class to store feed information in the Datastore. FeedInfo objects
 * hold the actual feed content as a serialized field (stored as a Blob in the
 * Datastore), along with some other bookkeeping information. (See the book
...
public class FeedInfo implements Serializable, Cacheable {
    ...
    private static final int DEFAULT_UPDATE_MINS = 2; // default latency
                                                      // interval before
        ...
        try {
            ...
            update_mins =
                Integer.parseInt(props
                    .getProperty("com.metadot.connectr.mins-feed-check"));
        } catch (Exception e) {
     ...
     */
    public SyndFeedInfo updateRequestedFeed(
    ...
    private void updateFeed(
        ...
        try {
            ...
            this.dateChecked = new Date();
            SyndFeedInfo fi =
                feedFetcher.retrieveFeedInfo(new URL(this.urlstring), this);
            // if non-null was returned, update the feed contents.
            // null indicates either that the feed did not need updating, or
            // that there was an error fetching it.
            if (fi != null) {
                this.feedInfo = fi;
                ...
                logger
                    .info("updating feed " + urlstring + " at " + dateUpdated);
            }
        } // end try
        catch (Exception e) {
            logger.warning(e.getMessage());
