...
/**
 * The FeedIndex persistence-capable class is used in conjunction with the
 * FeedInfo class to store feed information in the Datastore. This class points
 * to the Friend object(s) for which this feed was specified. (See the book
 * which accompanies this app for more information about the FeedIndex/FeedInfo
 * design).
 */
...
public class FeedIndex implements Serializable {
    ...
    private void addFriendKey(
        String fkey) {
        this.friendKeys.add(fkey);
    }
    
    // used for existing URLs lists.
    public static Set<String> updateFeedURLs(
        Set<String> newurls, Set<String> origUrls, String id, boolean replace) {
     ...
     */
    public static Set<String> addFeedURLs(
        Set<String> urls, String id) {
    ...
    public static FeedIndex findFeedIndex(
        String url, PersistenceManager pm) {
        FeedIndex fi = null;
    ...
    public static void removeFeedsFriend(
        Set<String> urls, String fkey) {
        for (String url : urls) {
            removeFeedFriend(url, fkey);
        }
    }
    
    public static void removeFeedFriend(
        String urlstring, String fkey) {
        ...
        try {
            ...
            if (findex != null) {
                ...
                if (fkeys.isEmpty()) {
                    ...
                    // then delete parent
                    pm.deletePersistent(pm.getObjectById(FeedInfo.class, findex
                        .getKey().getParent()));
    ...
    public static Set<String> addFeedsFriend(
        Set<String> urls, String fkey) {
        ...
        for (String url : urls) {
            try {
                ...
                if (status == false) { // then malformed url or some issue with
                                       // the feed
                    badUrls.add(url);
                }
            } catch (Exception e) { // also treat url as bad if anything else
                                    // goes wrong
                // [if timeout issue on initial add, user can try again to add
                // feed ]
                badUrls.add(url);
            }
        } // end for
    ...
    /**
     * build a new feed object if necessary, otherwise add fkey to existing
     * feedinfo object
     */
    public static boolean addFeedFriend(
        String urlstring, String fkey) {
        ...
        try {
            // first just check if the url is valid -- if it's not, nothing else
            // to do.
